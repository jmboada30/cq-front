...
import useUserRegistrationMutation from '../composables/useUserRegistrationMutation';

const isPwd = ref(true);
const { userRegistrationMutation, registerForm, clearRegisterForm } = useUserRegistrationMutation();
const { isRequired, isPassword, isEmail } = useValidators();
const { isDarkMode } = useUi();

const onSubmit = async () => {
  await userRegistrationMutation.mutateAsync(registerForm.value);
};

onBeforeUnmount(() => clearRegisterForm());
</script>
